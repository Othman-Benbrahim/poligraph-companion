...
const DB_VERSION = 9; // v9 : réseaux sociaux Wikidata (ré-enrichissement)
...
const PARLEMENT_INDEX_VERSION = 5; // v5 : pages officielles AN/Sénat
...
/* -------------------- détection de noms (pages web) ---------------- */

/**
 * Index compact { nomPlié → mini-fiche } pour le content script.
 * Limité aux profils Poligraph (les 35 000 maires du RNE créeraient
 * trop de faux positifs sur des noms courants).
 */
export async function buildNameIndex() {
  ...
  for (const p of all) {
    ...
    index[fold(name)] = {
      n: name,
      p: p.party || p.wikidataParty || "",
      m: p.mandateTitle || p.mandate || "",
      a: p.affairsCount || 0,
      f: p.factchecksCount || 0,
      u: p.profileUrl || `https://poligraph.fr/id/${encodeURIComponent(p.poligraphId)}`,
    };
  }
  return index;
}
...
/** Élus suivis : { poligraphId: { name } } dans storage.local. */
export async function getFollows() {
  const { follows } = await browser.storage.local.get("follows");
  return follows ?? {};
}

export async function toggleFollow(poligraphId, name) {
  const follows = await getFollows();
  if (follows[poligraphId]) delete follows[poligraphId];
  else follows[poligraphId] = { name };
  await browser.storage.local.set({ follows });
  return !!follows[poligraphId];
}

/**
 * Vérifie les flux RSS Poligraph et retient les items mentionnant un
 * élu suivi. Déduplication par lien (mémoire des 600 derniers vus).
 * @returns {Promise<number>} nombre de nouveaux items retenus
 */
export async function checkFeedsForFollows() {
  const follows = await getFollows();
  const names = Object.values(follows).map((f) => ({ raw: f.name, folded: fold(f.name) }));
  if (names.length === 0) return 0;

  const { fetchFeeds } = await import("./rss.js");
  const items = await fetchFeeds();

  const seen = new Set((await getMeta("rss:seen")) ?? []);
  const stored = (await getMeta("suivi:items")) ?? [];
  let added = 0;
  ...
  for (const item of items) {
    ...
    const match = names.find((n) => hay.includes(n.folded));
    if (!match) continue;
    stored.unshift({ ...item, matchedName: match.raw, unread: true, ts: Date.now() });
    added++;
  }

  await setMeta("rss:seen", [...seen].slice(-600));
  await setMeta("suivi:items", stored.slice(0, 50));
  return added;
}

export async function getSuiviItems() {
  return (await getMeta("suivi:items")) ?? [];
}

export async function markSuiviRead() {
  const items = (await getMeta("suivi:items")) ?? [];
  await setMeta("suivi:items", items.map((i) => ({ ...i, unread: false })));
}

export async function unreadSuiviCount() {
  const items = (await getMeta("suivi:items")) ?? [];
...
/**
 * Résumé des rubriques d'une déclaration (XML à la demande).
 * Succès en cache 30 j, échecs 24 h (XML pas toujours publié).
 * @returns {Promise<Array|null>} rubriques, ou null si indisponible
 */
export async function hatvpSummaryFor(declKey, nomFichier) {
  const cached = await getMeta(`hatvpxml:${declKey}`);
  if (cached) {
    if (cached.summary && Date.now() - cached.ts < MONTH_MS) return cached.summary;
    if (!cached.summary && Date.now() - cached.ts < DAY_MS) return null;
  }
  const { fetchDeclarationXml, parseDeclarationSummary } = await import("./hatvp.js");
  const xml = await fetchDeclarationXml(nomFichier);
  const summary = xml ? parseDeclarationSummary(xml) : null;
  await setMeta(`hatvpxml:${declKey}`, { ts: Date.now(), summary });
  return summary;
}
...
/* ------------- découverte de flux RSS (sites personnels) ----------- */

/** Flux RSS/Atom d'un site personnel — cache 30 j, échecs 24 h.
 *  Prérequis : la permission d'hôte pour ce site doit avoir été
 *  accordée par l'utilisateur (geste explicite dans le popup). */
export async function feedFor(siteUrl) {
  const key = `feed:${new URL(siteUrl).origin}`;
  const cached = await getMeta(key);
  if (cached) {
    if (cached.feed && Date.now() - cached.ts < MONTH_MS) return cached.feed;
    if (!cached.feed && Date.now() - cached.ts < DAY_MS) return null;
  }
  const { discoverFeed } = await import("./feedfind.js");
  const feed = await discoverFeed(siteUrl);
  await setMeta(key, { ts: Date.now(), feed });
  return feed;
}
