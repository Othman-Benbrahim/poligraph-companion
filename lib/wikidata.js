...
export async function resolveParties(qids) {
  ...
  for (let i = 0; i < qids.length; i += BATCH) {
    ...
    for (const [qid, entity] of Object.entries(entities)) {
      ...
      /* Réseaux sociaux (identifiants externes → URLs canoniques) */
      const tw = pickStringClaim(entity.claims, "P2002"); // X / Twitter
      const fb = pickStringClaim(entity.claims, "P2013"); // Facebook
      const ig = pickStringClaim(entity.claims, "P2003"); // Instagram
      const md = pickStringClaim(entity.claims, "P4033"); // Mastodon (user@instance)
      const socials = {};
      if (tw) socials.twitter = `https://x.com/${encodeURIComponent(tw)}`;
      if (fb) socials.facebook = `https://www.facebook.com/${encodeURIComponent(fb)}`;
      if (ig) socials.instagram = `https://www.instagram.com/${encodeURIComponent(ig)}`;
      if (md && md.includes("@")) {
        const [user, instance] = md.replace(/^@/, "").split("@");
        if (user && instance) socials.mastodon = `https://${instance}/@${user}`;
      }

      if (partyQid) partyQids.add(partyQid);
      if (partyQid || website || email || Object.keys(socials).length) {
        perPolitician.set(qid, { partyQid, website, email, socials });
      }
    }
  }
